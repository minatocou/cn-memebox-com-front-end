	...
	var Scroll = $.Class.extend({
		...
		_getTranslateStr: function(x, y) {
			if (this.options.hardwareAccelerated) {
				...
				// return 'translate3d(' + x*0.0625 + 'rem,' + y*0.0625 + 'rem,0px) ' + this.translateZ;
			}
			return 'translate(' + x + 'px,' + y + 'px) ';
			// return 'translate(' + x*0.0625 + 'rem,' + y*0.0625 + 'rem) ';
		},
		//API
	...
	Indicator.prototype = {
		...
		refresh: function() {
			...
			if (this.options.listenX) {
				...
				this.indicatorStyle.width = this.indicatorWidth*0.0625 + 'rem';
			...
			if (this.options.listenY) {
				...
				this.indicatorStyle.height = this.indicatorHeight*0.0625 + 'rem';
		...
		updatePosition: function() {
			...
			if (x < this.minBoundaryX) {
				...
				this.indicatorStyle.width = this.width*0.0625 + 'rem';
				x = this.minBoundaryX;
			...
			if (y < this.minBoundaryY) {
				...
				this.indicatorStyle.height = this.height*0.0625 + 'rem';
				y = this.minBoundaryY;
...
(function($, window) {
	...
	var Slider = $.Slider = $.Scroll.extend({
		...
		_fixedSlideNumber: function(page) {
			page = page || this.currentPage[0] || this.currentPage;
		...
		_gotoItem: function(slideNumber, time) {
			...
			this.currentPage = this._getPage(slideNumber, true); //此处传true。可保证程序切换时，动画与人手操作一致(第一张，最后一张的切换动画)
				this.scrollTo(this.currentPage[0].x, 0, time, this.options.scrollEasing);
				if (time === 0) {
					$.trigger(this.wrapper, 'scrollend', this);
				}
		...
		_initPulldownRefreshEvent: function() {
			...
			if (self.topPocket && self.options.webviewId) {
				$.plusReady(function() {
					...
					webview.setBounce({
						position: {
							top: height * 2*0.0625 + 'rem'
						},
						changeoffset: {
							top: height*0.0625 + 'rem'
						}
					});
				});
			}
		...
		pulldownLoading: function() { //该方法是子页面调用的
			$.plusReady(function() {
				plus.webview.currentWebview().setBounce({
					offset: {
						top: this.options.down.height*0.0625 + "rem"
					}
				});
			}.bind(this));
		},
		...
		setTranslateX: $.animationFrame(function(x) {
			if (this.scroller) {
				if (this.scalable && this.offCanvas.parentNode === this.wrapper) {
					...
					// this.scroller.style.webkitTransform = 'translate3d(' + x*0.0625 + 'rem,0,0) scale(' + zoomOutScale + ')';
				} else {
					if (this.slideIn) {
						this.offCanvas.style.webkitTransform = 'translate3d(' + x + 'px,0,0)';
						// this.offCanvas.style.webkitTransform = 'translate3d(' + x*0.0625 + 'rem,0,0)';
					} else {
						this.scroller.style.webkitTransform = 'translate3d(' + x + 'px,0,0)';
						// this.scroller.style.webkitTransform = 'translate3d(' + x*0.0625 + 'rem,0,0)';
					}
				}
			}
		}),
...
(function($, window, document, name) {
	...
	var backdrop = (function() {
		...
		element.addEventListener('tap', function(e) {
			...
			if (popover && $.targets.type=='toggle') {
	...
	var setStyle = function(popover, display, top, left) {
		...
		if (typeof top !== 'undefined')
			style.top = top*0.0625 + 'rem';
		if (typeof left !== 'undefined')
			style.left = left*0.0625 + 'rem';
	};
	var calPosition = function(popover, anchor, isActionSheet) {
		...
		if (position === 'top' || position === 'bottom') {
			...
			if (arrow) {
				...
				arrow.setAttribute('style', 'left:' + arrowLeft*0.0625 + 'rem');
			}
...
(function($, window, name) {
	...
	Toggle.prototype.toggle = function(animate) {
		...
		if (classList.contains(CLASS_ACTIVE)) {
			...
			this.handle.style.webkitTransform = 'translate(' + this.handleX*0.0625 + 'rem,0)';
		}
		$.trigger(this.element, 'toggle', {
			isActive: this.classList.contains(CLASS_ACTIVE)
		});
	...
	Toggle.prototype.setTranslateX = $.animationFrame(function(x) {
		...
		if (this.lastChanged !== isChanged) {
			if (isChanged) {
				this.handle.style.webkitTransform = 'translate(' + (this.initialState ? 0 : this.handleX*0.0625) + 'rem,0)';
				this.classList[this.initialState ? 'remove' : 'add'](CLASS_ACTIVE);
			} else {
				this.handle.style.webkitTransform = 'translate(' + (this.initialState ? this.handleX*0.0625 : 0) + 'rem,0)';
...
(function($, window, document) {
	...
	var setTranslate = function(element, x) {
		if (element) {
			element.style.webkitTransform = 'translate(' + x*0.0625 + 'rem,0)';
		}
	};
	...
	Input.prototype.initElementEvent = function() {
		...
		if (this.sliderActionClass) {
			...
			var showTip = function() { //每次重新计算是因为控件可能被隐藏，初始化时计算是不正确的
				...
				tooltip.style.left = (14 + offsetLeft + scaleWidth - tooltipWidth / 2)*0.0625 + 'rem';
