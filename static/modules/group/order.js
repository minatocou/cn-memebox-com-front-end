    ...
    methods: {
        ...
        appPayCall: function (data) {
            if (data && data.code == 1 && data.data && data.data.orderId) {
                this.finishReport();
                localStorage.grouponId = data.data.orderId;
                location.href = location.origin + this.page + '/payment/group.html?orderId=' + data.data.orderId;
            } else {
        ...
        isShowID: function () {
             ...
             */
            return true;
            // return this.pageData.isGlobalProduct == 1 || this.pageData.isFTZProduct == 1;
        },
        checkout: function () {
            var $this = this;
            console.log(this.pageData.stockStatus, 123)
            ...
            if ($this.orderAddress && $this.orderAddress.addressId) {
                if (!$this.orderAddress.idcard && $this.isShowID()) {
                    ...
                    var activityId = $this.getSearch('activityId');
                    var remark = 'h5';
                    if ($this.isIosApp()) {
                        remark = 'ios';
                    ...
                    $this.httpAjax({
                        url: '/h5/newgrouponcheckout/placeOrder',
                        param: param,
                        success: function (data) {
                            if (data.code == 15) {
                                setTimeout(function () {
